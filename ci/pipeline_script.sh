...
echo "Linting the OAS spec using inso-cli"

inso lint spec spec/swagger-petstore.yaml
...
deck diff -s $repository_path/meta/meta.yaml -s ${tmp_path}/kong.yaml -s $repository_path/plugins/ --workspace $kong_workspace --headers "kong-admin-token:$admin_token" --kong-addr ${admin_api} --tls-skip-verify

echo "Deploying API to Kong"

deck sync -s $repository_path/meta/meta.yaml -s ${tmp_path}/kong.yaml -s $repository_path/plugins/ --workspace $kong_workspace --headers "kong-admin-token:$admin_token" --kong-addr ${admin_api} --tls-skip-verify
